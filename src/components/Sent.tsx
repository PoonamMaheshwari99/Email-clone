...
//const totalPages = MOCK_DATA.emails.filter(MOCK_DATA=> {return( MOCK_DATA.type==="Sent")&& (MOCK_DATA.isDeleted===false)}).length;
...
const fetchHandler=async()=>{
  ...
  const response = await fetch('http://localhost:3000/emails');
      ...
      setTotalPages(resdata.filter((data1: { type: string; isDeleted: boolean; })=> {return( data1.type==="Sent")&& (data1.isDeleted===false)}).length)
 ...
 console.log(resdata);
  ...
  const transformeddata:EmailClass[] = resdata.filter((data1: any)=> {return( data1.type==="Sent")&& (data1.isDeleted===false)})
  ...
  const transformeddata1:EmailClass[] = resdata.filter((data1: any)=> {return( data1.type==="Sent")&& (data1.isDeleted===false)})
...
useEffect(()=>{
  ...
  /*const fetchHandler= ()=>{
   ...
   const transformeddata:List[] = MOCK_DATA.emails.filter(MOCK_DATA=> {return( MOCK_DATA.type==="Sent")&& (MOCK_DATA.isDeleted===false)})
    .slice(pageVisited, pageVisited + perPage)
    .map((Email) => {
   ...
   const onChangeRight = () => {
       ...
       const handleSearch= (e:any) => {
        ...
        if(e.target.value === ''){
          ...
          const filterResult = searchData.filter(item => item.to.toLowerCase().includes(e.target.value.toLowerCase()));
